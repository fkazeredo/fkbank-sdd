---
name: design-slice
description: RELAY phase — budgeted reading, full Decision Ladder, executable plan and one-session fit check for a READY spec (R2+). Never changes production code. Manual invocation only.
...
5. Write `.claude/runtime/<id>/plan.md`: Understanding · Modules touched · Files ·
   ...
   · Decision Ladder record · **One-session fit check**: "Can this slice be implemented and
   verified in one build session without expected compaction?" — check the slice-too-big
   signals in decision-ladder.md; if it does not fit, propose the split BEFORE approval
   (a slice that does not fit the session is a slice problem, not a session problem).
...
7. Validate that every plan decision is strictly derived from the approved spec and durable
   architecture. If so, state `PLAN_APPROVED` and continue automatically. A new material
   choice becomes `HUMAN_DECISION_REQUIRED`.
