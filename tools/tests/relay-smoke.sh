...
run machine-orchestration python3 -c 'import json,sys,pathlib; r=pathlib.Path(sys.argv[1]); d=json.load(open(r/".claude/settings.json")); expected={"Agent(qa-engineer)","Agent(pr-reviewer)","Agent(security-assurance-engineer)"}; assert expected <= set(d["permissions"]["allow"]); assert not expected & set(d["permissions"]["deny"]); assert all((r/".claude/skills"/x/"SKILL.md").is_file() for x in ("deliver-spec","deliver-sprint","close-sprint","security-assurance"))' "$ROOT"
